...
jointCounts = table(accidentsData$INT_HWY, accidentsData$INJURY_CRASH)
jointDist = t(t(jointCounts)/colSums(jointCounts))
...
nb.train <- function(D = NULL){
  ...
  for (j in 1:nAttr){
    ...
    if( min(table(D[,cRow],D[,j])) == 0){
      print(table(D[,cRow],D[,j]))
      tmpAdist = table(D[,cRow],D[,j]) + baysianIncrease #adding 1
      print(tmpAdist)
      #nRowNew = nRow+baysianIncrease*dim(tmpAdist)[1]*dim(tmpAdist)[2]
#       tmpAdist = tmpAdist/sum(tmpAdist) 
#       
#       print(tmpAdist)
#       print(rowSums(tmpAdist))
      
      print("you cocksucker")
#       print(tmpAdist)
#       print(table(D[,cRow],D[,j]))
#       print(colSums())
    }
    else{
      tmpAdist = table(D[,cRow],D[,j])
      #tmpAdist = table(D[,cRow],D[,j])/nRow      # Pr(A_i,C)
    }
    
    tmpAdist = t(t(tmpAdist)/rowSums(tmpAdist))
